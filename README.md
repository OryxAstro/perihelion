...
Non-sidereal tracking for comets and asteroids across three real front ends: [PINS](https://github.com/nitr57/pins) (the Raspberry Pi fork of N.I.N.A.) with its [Touch-N-Stars](https://github.com/Touch-N-Stars/Touch-N-Stars) companion app, and a genuinely separate **native build for real Windows N.I.N.A.** — a dockable panel plus a standalone Framing Composer window, for users who never touch PINS at all. One shared orbital-mechanics/tracking core; two independent UI layers on top of it.
...
Perihelion closes that gap for PINS with no shared code, cache format, or namespace with Orbitals — computing tracking rates in-process (no external service, no internet dependency for the tracking math itself) and shipping its own Touch-N-Stars panel. The same core also ships as a native Windows plugin for real NINA users, independent of PINS entirely — see [Native Windows NINA plugin](#native-windows-nina-plugin) below.
...
- A real sky-survey image (NASA/HIPS2FITS/STSCI/ESO/SkyServer) centered on the target, with the camera's real FOV rectangle overlaid, scaled and sized from the connected profile's own gear settings.
