...
**Why the lists are short**: comets are filtered to those currently brighter than magnitude 16 against the live MPC feed, capped at 30 shown — the number visible on any given day (often around a dozen) reflects how many are actually worth pointing a telescope at, not a limitation of the fetch. The 13 asteroids are every one bright enough for a typical amateur setup to realistically image — a short, curated list beats downloading an entire minor-planet catalog (well over a million objects, the overwhelming majority magnitude 18+ and invisible to any amateur rig).
